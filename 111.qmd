...
output_folder <- "processedd_datasets"

# Создаем папку для сохранения обработанных данных, если ее нет
...
for (file in files) {
  ...
  required_columns <- c("IdSubscriber", "date", "Hour", "UpTx", "DownTx")
  if (!all(required_columns %in% colnames(data))) {
    cat("Пропущены необходимые колонки в файле:", file, "\n")
    next
  }
  
  # Рассчитываем среднее и стандартное отклонение для UpTx и DownTx
  ...
  # Определяем аномалии
  data$is_hacked <- (abs(data$UpTx - up_mean) > 3 * up_sd) | 
                    (abs(data$DownTx - down_mean) > 3 * down_sd)
  
  # Выбираем только нужные колонки
  result <- data[, c("IdSubscriber", "date", "Hour", "UpTx", "DownTx", "is_hacked")]
  
  # Сохраняем обработанный файл в новую папку
