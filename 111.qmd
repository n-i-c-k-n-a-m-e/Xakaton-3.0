...
```
#чтенине какого-либо из дней
```{r}
# Чтение CSV файла
possible_hacks_hourly <- read.csv("possible_hacks_hourly.csv")

# Открытие таблицы в отдельном окне
View(possible_hacks_hourly)
...
```
#Выявление сттранных пользователей
```{r}
# Загрузка библиотек
library(dplyr)
library(lubridate)
...
# Преобразование столбца StartSession в формат даты/времени
data$StartSession <- dmy_hms(data$StartSession)

# Добавление столбца для группировки по часам
data <- data %>%
  mutate(HourGroup = floor_date(StartSession, unit = "hour"))

# Вычисление статистики (среднее и стандартное отклонение) для каждого пользователя за каждый час
hourly_stats <- data %>%
  group_by(IdSubscriber, HourGroup) %>%
  summarise(
    AvgUpTx = mean(UpTx, na.rm = TRUE),
    SdUpTx = sd(UpTx, na.rm = TRUE),
    AvgDownTx = mean(DownTx, na.rm = TRUE),
    SdDownTx = sd(DownTx, na.rm = TRUE)
  ) %>%
  ungroup()

# Объединение исходных данных со статистикой
data <- data %>%
  left_join(hourly_stats, by = c("IdSubscriber", "HourGroup"))

# Определение аномалий: если трафик превышает среднее + 3 стандартных отклонения
data <- data %>%
  mutate(
    IsAnomalyUpTx = ifelse(UpTx > (AvgUpTx + 3 * SdUpTx), TRUE, FALSE),
    IsAnomalyDownTx = ifelse(DownTx > (AvgDownTx + 3 * SdDownTx), TRUE, FALSE),
    PossiblyHacked = ifelse(IsAnomalyUpTx | IsAnomalyDownTx, TRUE, FALSE)
  )

# Фильтрация данных для выявления возможных взломов
hacked_users <- data %>%
  filter(PossiblyHacked) %>%
  select(IdSubscriber, StartSession, UpTx, DownTx, AvgUpTx, AvgDownTx, IsAnomalyUpTx, IsAnomalyDownTx)

# Сохранение списка возможных взломов в файл (опционально)
write.csv(hacked_users, "possible_hacks.csv", row.names = FALSE)

# Вывод списка возможных взломов на экран
print(hacked_users)
